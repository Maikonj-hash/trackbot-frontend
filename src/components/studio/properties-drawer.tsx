    ...
    return (
        <div className="w-72 h-full border-l border-border/50 bg-background/95 shadow-2xl flex flex-col z-10 transition-transform duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {(selectedNode.type === "messageBlock" || selectedNode.type === "optionsBlock" || selectedNode.type === "inputBlock" || selectedNode.type === "identificationBlock") && (
                    <div className="space-y-3">
                        <label className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">
                            {selectedNode.type === "identificationBlock" ? "Mensagem de Identificação" : "Texto da Mensagem / Pergunta"}
                        </label>
                        <textarea
                            ...
                            placeholder={selectedNode.type === "identificationBlock" ? "Olá! Para começarmos, identifique-se por favor:" : "O que o bot deve falar?"}
                        />
                    </div>
                )}

                {selectedNode.type === "identificationBlock" && (
                    <div className="space-y-4 pt-4 border-t border-border/50">
                        <div className="flex items-center justify-between">
                            ...
                            <button
                                onClick={() => {
                                    const currentFields = selectedNode.data.identificationFields || [];
                        ...
                        <div className="space-y-3">
                            {(selectedNode.data.identificationFields || []).map((field: any, index: number) => (
                                <div key={index} className="p-3 bg-muted/20 rounded-lg border border-border/50 space-y-2 relative group">
                                    <button
                                        onClick={() => {
                                            const newFields = [...(selectedNode.data.identificationFields || [])];
                                            newFields.splice(index, 1);
                                            updateNodeData(selectedNode.id, { identificationFields: newFields });
                                        }}
